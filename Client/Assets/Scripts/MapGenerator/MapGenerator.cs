...
namespace GameEngine.MapGenerator
{
    ...
    {
        private int maxDepth = 5; //방의 개수 : 2^ maxNode
        ...
        public void DivideTree(TreeNode treeNode, int depth) //재귀 함수
        {
            if (depth < maxDepth) //0 부터 시작해서 노드의 최댓값에 이를 때 까지 반복
            {
                ...
                int split = (int)Random.Range(length * 0.4f, length * 0.6f); //기준선 위에서 최소 범위와 최대 범위 사이의 값을 무작위로 선택
                ...
                if (size.width >= size.height) //가로
                {
                    ...
                    treeNode.LeftNode.Name = treeNode.Name + "- L";
                    ...
                    treeNode.RightNode.Name = treeNode.Name + "- R";
        ...
        public RectInt GenerateDungeon(TreeNode treeNode, int depth) //방 생성
        {
            ...
            // {
            //     GenerateDungeon(treeNode.LeftNode, depth + 1);
            //     GenerateDungeon(treeNode.RightNode, depth + 1);
            //     return default;
            // }
            ...
            if (depth == maxDepth) //노드가 최하위일 때만 조건문 실행
            {
                ...
                int width = (int)Random.Range(size.width * 0.6f, size.width - 1); //트리 범위 내에서 무작위 크기 선택, 최소 크기 : width / 2
                int height = (int)Random.Range(size.height * 0.6f, size.height - 1);
            ...
            treeNode.DungeonSize = treeNode.RoomSize;
        ...
        public void GenerateRoad(TreeNode treeNode, int n) //길 연결
        {
            ...
            var leftNodeCenter = treeNode.LeftNode.GetCenterInt();
            var rightNodeCenter = treeNode.RightNode.GetCenterInt();
